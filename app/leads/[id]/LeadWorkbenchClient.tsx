...
import EngagedStageWorkspace from '@/components/leads/EngagedStageWorkspace';
...
export default function LeadWorkbenchClient({ initialLead, initialActions, initialContacts }: WorkbenchProps) {
  ...
  // 💎 NEW: Filter notes out of the unified action ledger array ordered chronologically
  const loggedNotes = actions.filter(a => a.type === 'note');

  return (
    <div className="space-y-6 max-w-[1600px] mx-auto p-4 md:p-6 text-foreground">
      <PipelineHeader lead={lead} actions={actions} contacts={contacts} setLead={setLead} />
        ...
        <div className="lg:col-span-8 space-y-4">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="workspace" className="space-y-4 mt-4 animate-fadeIn">
              ...
              {/* 💎 FIXED: Engaged Stage Workspace Connected Natively */}
              {lead.status === 'engaged' && (
                <EngagedStageWorkspace
                  lead={lead}
                  actions={actions}
                  contacts={contacts}
                  onActionUpdated={handleActionUpdated}
              ...
              {/* Keep other layout compilation block states safe */}
              {['solution_fit', 'proposal', 'negotiation', 'close'].includes(lead.status) && (
                <div className="bg-card p-6 border border-border/60 rounded-xl text-center text-xs font-semibold text-muted-foreground">
                  {lead.status.toUpperCase()} Workspace stage view active.
                </div>
              )}

              <NotesWidget leadId={lead.id} currentStage={lead.status} onNoteSaved={handleActionCreated} />
            </TabsContent>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  <div className="flex items-center gap-2.5 p-2.5 bg-muted/20 border border-border/40 rounded-lg">
                    ...
                    <div className="truncate">
                      ...
                      {lead.website ? <a href={`https://${lead.website}`} target="_blank" rel="noreferrer" className="font-semibold text-primary hover:underline">{lead.website}</a> : <span className="text-muted-foreground">Unlisted</span>}
                    </div>
                  </div>
                  <div className="flex items-center gap-2.5 p-2.5 bg-muted/20 border border-border/40 rounded-lg">
              ...
              {pendingTasks.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {pendingTasks.slice(0, 2).map(t => (
                    <div key={t.id} className="p-2 bg-muted/30 border border-border/40 rounded-lg font-medium text-foreground truncate">• {t.title}</div>
                  ))}
                </div>
              )}
            </div>
              ...
              {recentNotifications.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {recentNotifications.map(n => (
                    <div key={n.id} className="p-2 bg-primary/5 border border-primary/10 rounded-lg text-foreground leading-relaxed">{n.body}</div>
                  ))}
                </div>
              )}
            </div>

            {/* 💎 NEW: CHRONOLOGICAL SAVED NOTES CONTAINER FEED */}
            <div className="space-y-2 pt-3 border-t border-border/40">
              <span className="font-bold text-muted-foreground uppercase tracking-wider block mb-1 flex items-center gap-1.5 select-none">
                <FileText className="h-4 w-4 text-primary" /> Saved Meeting & Scratch Notes ({loggedNotes.length})
              </span>
              {loggedNotes.length === 0 ? (
                <span className="text-muted-foreground block py-1 italic">No text notes saved yet.</span>
              ) : (
                <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
                  {loggedNotes.map((note) => (
                    <div key={note.id} className="p-2.5 bg-muted/30 border border-border/50 rounded-lg space-y-1 text-foreground leading-relaxed animate-fadeIn">
                      <div className="flex items-center justify-between text-[9px] font-bold text-primary uppercase select-none">
                        <span>{note.pipeline_stage || 'General'}</span>
                        <span className="text-muted-foreground">{note.created_at ? new Date(note.created_at).toLocaleDateString() : ''}</span>
                      </div>
                      <p className="text-[11px] font-medium text-foreground/90 whitespace-pre-wrap">{note.body}</p>
                    </div>
                  ))}
                </div>
              )}
